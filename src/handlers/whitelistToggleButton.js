...
/**
 * Handles whitelist toggle confirmation buttons
 * @param {import('discord.js').ButtonInteraction} interaction - The button interaction
 * @returns {Promise<void>}
 */
export async function handleWhitelistToggleButton(interaction) {
    try {
        // Extrair ação e userId do customId
        // Formato: wl_confirm_activate_123456789 ou wl_confirm_deactivate_123456789
        const customIdParts = interaction.customId.split('_');
        if (customIdParts.length < 4) {
            logger.error('Custom ID inválido para toggle de whitelist', {
        ...
        const currentState = db.isSystemEnabled(guildId, 'whitelist');
        
        if (isActivate && currentState) {
            return await interaction.update(error({
                title: 'Sistema Já Está Ativo',
                description: 'O sistema de whitelist já está ativado.',
                components: []
            }));
        }
        ...
        if (!isActivate && !currentState) {
            return await interaction.update(error({
                ...
                description: 'O sistema de whitelist já está desativado.',
                components: []
            }));
        }

        // Executar ação
        db.setSystemEnabled(guildId, 'whitelist', isActivate);

        logger.info(`Sistema de whitelist ${isActivate ? 'ativado' : 'desativado'}`, {
            guildId,
        ...
        const embed = new EmbedBuilder()
            ...
            .setTitle(isActivate ? '✅ Sistema de Whitelist Ativado' : '🔴 Sistema de Whitelist Desativado')
            .setDescription(
                isActivate
                    ? 'O sistema de whitelist foi **ativado** com sucesso! Agora os usuários podem solicitar whitelist.'
                    : 'O sistema de whitelist foi **desativado**. Os usuários não poderão mais solicitar whitelist até que seja reativado.'
            )
        ...
        logger.error('Erro ao processar botão de toggle de whitelist', {
            error: error.message,
            stack: error.stack,
            customId: interaction.customId,
            userId: interaction.user.id
